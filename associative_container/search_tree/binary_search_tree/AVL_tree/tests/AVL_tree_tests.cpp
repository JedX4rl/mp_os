        ...
        std::vector<typename AVL_tree<tkey, tvalue>::iterator_data> &expected_result)
{
    ...
    {
        bool a = (*it)->get_depth() != item.get_depth();
        a = (*it)->get_key() != item.get_key();
        a = (*it)->get_value() != item.get_value();
        a = reinterpret_cast<typename AVL_tree<tkey, tvalue>::iterator_data const *>(*it)->get_subtree_height() != item.get_subtree_height();
