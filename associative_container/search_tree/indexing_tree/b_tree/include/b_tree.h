...
public:
    ...
    public:
        ...
        explicit infix_iterator(
                std::stack<std::pair<typename search_tree<tkey, tvalue>::common_node *, int>>);
...
{
    auto data = *_iterator;
    auto n =std::tuple<size_t, size_t, tkey const &, tvalue const &>(std::get<0>(data), std::get<1>(data), std::cref(std::get<2>(data)), std::cref(std::get<3>(data)));
    return n;
}
        ...
        search_tree<tkey, tvalue>(keys_comparer, allocator, logger)
{
    if ((_t = t) < 2)
    {
        throw std::logic_error("Invalid value of t");
    }
}
...
{
    return b_tree<tkey, tvalue>::infix_const_iterator(reinterpret_cast<typename search_tree<tkey, tvalue>::common_node*>(this->_root));
}
...
void b_tree<tkey, tvalue>::clear()
{
    clear(reinterpret_cast<typename search_tree<tkey, tvalue>::common_node*>(this->_root));
}
